...
describe('BookingService', () => {
    let service;
    beforeEach(() => {
        service = new bookings_service_1.BookingService();
        jest.clearAllMocks();
    });
    describe('checkSlotOverlap', () => {
        it('returns true when the incoming window overlaps an existing slot', async () => {
            ...
            const result = await service.checkSlotOverlap('truck-1', new Date('2026-02-10T12:00:00Z'), new Date('2026-02-10T16:00:00Z'));
        ...
        it('returns false when the incoming window does not overlap', async () => {
            ...
            const from = jest.fn().mockReturnValue({ where });
            db_1.db.select.mockReturnValue({ from });
            const result = await service.checkSlotOverlap('truck-1', new Date('2026-02-10T14:00:00Z'), new Date('2026-02-10T16:00:00Z'));
        ...
        it('ignores overlap on the excluded slot id', async () => {
            ...
            const from = jest.fn().mockReturnValue({ where });
            db_1.db.select.mockReturnValue({ from });
            const result = await service.checkSlotOverlap('truck-1', new Date('2026-02-10T12:00:00Z'), new Date('2026-02-10T13:00:00Z'), 'slot-1');
            expect(result).toBe(false);
        });
    });
    describe('validateTransition', () => {
        it('allows valid state transitions', () => {
            expect(() => service['validateTransition'](bookings_service_1.BookingStatus.PENDING_DEPOSIT, bookings_service_1.BookingStatus.CONFIRMED)).not.toThrow();
            expect(() => service['validateTransition'](bookings_service_1.BookingStatus.CONFIRMED, bookings_service_1.BookingStatus.RUNNING)).not.toThrow();
        });
        it('rejects invalid state transitions', () => {
            expect(() => service['validateTransition'](bookings_service_1.BookingStatus.PENDING_DEPOSIT, bookings_service_1.BookingStatus.COMPLETED)).toThrow(/Invalid transition/);
            expect(() => service['validateTransition'](bookings_service_1.BookingStatus.COMPLETED, bookings_service_1.BookingStatus.RUNNING)).toThrow(/Invalid transition/);
        });
    });
    describe('assignDriverToBooking', () => {
        it('assigns a driver user to booking', async () => {
            ...
            db_1.db.update.mockReturnValue({ set });
            const result = await service.assignDriverToBooking('booking-1', 'driver-1');
        ...
        it('rejects assigning non-driver user', async () => {
            ...
            db_1.db.select
                .mockReturnValueOnce({ from: bookingFrom })
                .mockReturnValueOnce({ from: driverFrom });
            await expect(service.assignDriverToBooking('booking-1', 'user-1')).rejects.toThrow('Assigned user must have driver role');
        });
    });
});
